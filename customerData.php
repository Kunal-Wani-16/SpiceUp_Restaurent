...
<?php
    if($_SERVER["REQUEST_METHOD"] == "POST") {  
        ...
        $sql = "UPDATE `booktable` SET `email`='$email',`name`='$name',`mobile`='$mobile',`message`='$message' WHERE email='$email'";
...
<!-- displaying title n desc in table -->
  ...
  $srno=1;
    ...
    $result = mysqli_query ($conn , $sql);
    if($result) {
        $sql = "SELECT `name`, `email`, `date`, `mobile`,`message` FROM `booktable` ";
    $result = mysqli_query ($conn , $sql);
